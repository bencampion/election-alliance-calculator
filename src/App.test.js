...
describe('[Lib Dem + Green] Left Alliance and [DUP + UUP] Right Alliance', () => {
  ...
  test('4 seats changed', () => {
    ...
    expect(screen.getAllByRole('listitem')[0]).toHaveTextContent('Carshalton and Wallington — London (Left Alliance gain from Conservative)');
    expect(screen.getAllByRole('listitem')[1]).toHaveTextContent('East Dunbartonshire — Scotland (Left Alliance gain from SNP)');
    expect(screen.getAllByRole('listitem')[2]).toHaveTextContent('North Down — Northern Ireland (Right Alliance gain from APNI)');
    expect(screen.getAllByRole('listitem')[3]).toHaveTextContent('Sheffield, Hallam — Yorkshire and The Humber (Left Alliance gain from Labour)');
  });

  test('query string is updated', () => {
    expect(window.location.search).toBe('?left=green&left=ld&right=dup&right=uup');
  });
